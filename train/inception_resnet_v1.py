...
def reduction_a(net, k, l, m, n):
    with tf.variable_scope('Branch_0'):
        tower_conv = slim.conv2d(net, n, 3, stride=2, padding='VALID',
                                 scope='Conv2d_1a_3x3')
    with tf.variable_scope('Branch_1'): # 在branch_1下命名的conv2d_0a_1x1
...
def reduction_b(net):
    ...
    return net
#  该函数是对inception_resent_v1输出后的进一步处理
...
def inception_resnet_v1(inputs, is_training=True,
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse): # 这里的scope是为了加上一个文件前缀名，tensor需要加上[]列表的形式
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training):
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                                stride=1, padding='SAME'):
      
                # 149 x 149 x 32 下面操作之后的shape
