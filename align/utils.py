...
def get_dataset(paths):
    ...
    for i in tqdm(range(nrof_classes)): # tqdm增加一个进度条信息
        class_name = classes[i]
        facedir = os.path.join(paths, class_name) # 得到文件夹路径下的某个人的路径 文件名+人类别名
        image_paths = get_image_paths(facedir) # 得到每个人文件路径下的每张图片的路径 文件名+人类别名+每张图片名
        dataset.append(ImageClass(class_name, image_paths)) # dataset是包含 ImageClass对象的集合
    return dataset

def get_image_paths(facedir):
    image_paths = []
    if os.path.isdir(facedir):
        images = os.listdir(facedir) # 返回指定文件名包含的文件名字的列表
        image_paths = [os.path.join(facedir,img) for img in images] # 得到每个人文件路径下的每张图片的路径
    return image_paths
...
def split_dataset(dataset,split_ratio,min_nrof_images_per_class):
    ...
    返回值：
      train_set,test_set:包含有图片类别和路径的训练验证集
    '''
    train_set=[]
    test_set=[]
    for cls in dataset:
...
def get_image_paths_and_labels(dataset):
    ...
    for i in range(len(dataset)):
        image_paths_flat+=dataset[i].image_paths  # 【文件名+人类名+图片名】
        labels_flat+=[i]*len(dataset[i].image_paths) # 【[人类名]*len】
...
def create_input_pipeline(input_queue,image_size,nrof_preprocess_threads,bath_size_placeholder):
    ...
    for _ in range(nrof_preprocess_threads):  # 分成4个线程同时运行
        filenames,label=input_queue.dequeue() # 一个个出队，出队的是每个人图片的路径名
        images=[]
        for filename in tf.unstack(filenames):
            file_contents=tf.read_file(filename)
            image=tf.image.decode_image(file_contents,3) # 将各个类型的图片解码成utf-8
            #随机翻转图像
            ...
            images.append(image)
        image_and_labels_list.append([images,label]) #分为4组，每组的数据是 将每个人的某张图片与标签加入到列表
    # batch_size大小（以每个标签为数据单位，一个标签对应一张图片）
    image_batch,label_batch=tf.train.batch_join(image_and_labels_list,
                                  batch_size=bath_size_placeholder,
                                  shapes=[image_size+(3,),()],
                                  enqueue_many=True,
                                  capacity=4*nrof_preprocess_threads*100, #最多能填充的个数
